<template>
  <div
    class="w-full shadow border-b border-gray-200 mb-8 sm:rounded-lg overflow-auto"
  >
    <table class="w-full divide-y divide-gray-200 table-auto">
        ...
        <tr>
          ...
          >
            <slot :name="'header-' + i">
              ...
              <div
                class="sort-arrows inline-block text-center absolute"
                v-if="sorting.hasOwnProperty(i) && sorting[i]"
              >
                <span
                  @click.prevent="onSortChange(i, 'asc')"
                  :class="sortControlClasses(i, 'asc')"
                  class="w-full block cursor-pointer font-normal hover:font-bold focus:font-bold hover:text-theme-600 focus:text-theme-600 dark:hover:text-theme-500 dark:focus:text-theme-500"
                  ><i class="fa fa-caret-up"></i
                ></span>
                <span
                  @click.prevent="onSortChange(i, 'desc')"
                  :class="sortControlClasses(i, 'desc')"
                  class="w-full block cursor-pointer font-normal hover:font-bold focus:font-bold hover:text-theme-600 focus:text-theme-600 dark:hover:text-theme-500 dark:focus:text-theme-500"
      ...
      <tbody
        v-if="records && records.length && !$props.isLoading"
        class="bg-white divide-y divide-gray-200"
      >
        <tr
          v-for="(record, i) in records"
          @click.prevent="onRecordClick(record)"
        ...
        >
          ...
          >
            <slot :name="'actions-' + j" v-for="(action, j) in actions">
              <router-link
                v-if="action.hasOwnProperty('to') && action.to"
                :to="getActionPage(action, record)"
                :class="getActionClass(action)"
                :title="action.name"
              >
                <i v-if="action.icon" :class="action.icon"></i>
                <span
                  v-if="!action.hasOwnProperty('showName') || action.showName"
                  v-html="action.name"
                ></span>
              </router-link>
              <a
                v-else
                :class="getActionClass(action)"
                @click="onActionClick({ action: action, item: record })"
                :title="action.name"
              >
                <i v-if="action.icon" :class="action.icon"></i>
                <span
  ...
  <Pager
    v-if="lastPage && lastPage > 1 && !$props.isLoading"
    :page-count="lastPage"
    :value="currentPage"
    @input="onPagerInput"
  />
...
export default defineComponent({
  ...
  emits: ["pageChanged", "action", "sort", "record-click"],
  props: {
    id: {
      type: String,
      default: "",
