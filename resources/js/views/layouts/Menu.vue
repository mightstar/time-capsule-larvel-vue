    ...
    <template v-for="item in $props.state.mainMenu">
      <li
        v-if="
          ...
          isEnabled(item, $props.type)
        "
      >
        <button
          @click.prevent="
            state.currentExpandedMenuItem
              ? (state.currentExpandedMenuItem = null)
              : (state.currentExpandedMenuItem = item)
          "
          type="button"
          class="transition duration-75 group w-full flex items-center p-2 text-base font-normal text-white font-semibold rounded-lg hover:text-theme-300 hover:bg-theme-800 dark:hover:bg-theme-800"
          :class="isActive(item) ? 'bg-theme-800' : ''"
        >
          <Icon :name="item.icon" class="mr-2 text-3xl pl-2 -mt-1" />
          <span class="flex-1 text-left" v-html="item.name"></span>
          <Icon
            :name="
              JSON.stringify(state.currentExpandedMenuItem) ===
              JSON.stringify(item)
                ? 'angle-up'
                : 'angle-down'
            "
            class="mr-2 text-3xl pl-2 -mt-1"
          />
        </button>
        <ul
          id="dropdown-example"
            ...
            <template v-if="isEnabled(subitem, $props.type)">
              <li v-if="subitem.hasOwnProperty('onClick')">
                <router-link
                  ...
                  class="flex items-center p-2 pl-11 w-full text-base font-normal text-white font-semibold rounded-lg hover:text-theme-300 hover:bg-theme-800 dark:hover:bg-theme-800"
                  :class="isActive(subitem) ? 'bg-theme-800' : ''"
                >
                  {{ subitem.name }}
      ...
      <li v-else-if="isEnabled(item, $props.type)">
        <a
          ...
          href="#"
          class="flex items-center p-2 text-base font-normal text-white font-semibold rounded-lg hover:text-theme-300 hover:bg-theme-800 dark:hover:bg-theme-800"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function isEnabled(obj, type) {
      ...
      if (type === "desktop") {
        return !!obj.showDesktop;
      } else if (type === "mobile") {
        return !!obj.showMobile;
      }
      return false;
    }
